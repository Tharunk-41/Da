...
def compare_and_highlight(before_file, after_file, key_column):
    df_before, df_after = pd.read_csv(before_file), pd.read_csv(after_file)
    df_merged = merge_dataframes(df_before, df_after, key_column)
...
def merge_dataframes(df_before, df_after, key_column):
    df_merged = pd.merge(df_before, df_after, on=key_column, how='outer', suffixes=('_before', '_after'), indicator=True)
    column_order = [key_column] + [f'{col}_before' for col in df_before.columns if col != key_column] + [f'{col}_after' for col in df_before.columns if col != key_column] + ['_merge']
...
if before_file:
    ...
    key_column = st.sidebar.selectbox("🔑 Select Key Column", df_before.columns)
else:
    key_column = None
...
with tab1:
    ...
    st.write("2. Select the key column for comparison.")
...
with tab2:
    if before_file and after_file and key_column:
        if st.button("🔍 Compare & Generate Report", use_container_width=True):
            with st.spinner("Processing... Please wait."):
                output_file = compare_and_highlight(before_file, after_file, key_column)
        ...
        st.warning("⚠️ Please upload both CSV files and select a key column to proceed.")
